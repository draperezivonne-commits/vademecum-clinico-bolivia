/* Vademécum Clínico Bolivia v0.20.8 · seguridad de presentación de interacciones */
(function(){
  ...
  function render(){
    ...
    el.querySelectorAll('.v207-sources').forEach(x=>x.remove());
    const note=el.querySelector('.v207-note');
    if(note)note.innerHTML='<b>Importante:</b> el resultado se refiere únicamente a los medicamentos escritos. Los productos asociados se evalúan por cada ingrediente identificado. Si la base no contiene una regla verificada, se informa “sin interacción detectada”, nunca “seguro”. Pueden influir dosis, edad, embarazo, función renal/hepática, QT, electrolitos y otras condiciones clínicas.';
  }
  ...
  if(panel){
    ...
    if(banner)banner.innerHTML='<b>Resultado clínico directo:</b> NO COMBINAR · EVITAR · MONITORIZAR · SIN INTERACCIÓN RELEVANTE DETECTADA. La ausencia de una regla no se presenta como “seguro”.';
  }
  const about=document.querySelector('#sincronizacion .about-card p b');
  if(about)about.innerHTML=about.innerHTML.replace(/Aplicación\s+0\.20\.\d+/,'Aplicación 0.20.8');
  window.VCB_INTERACTION_UI_VERSION='0.20.8';
})();
